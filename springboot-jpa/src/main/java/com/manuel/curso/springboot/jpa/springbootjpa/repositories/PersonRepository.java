package com.manuel.curso.springboot.jpa.springbootjpa.repositories;
...
public interface PersonRepository extends CrudRepository<Person, Long> {
    ...
    @Query("select p from Person p where p.name between ?1 and ?2 order by p.name asc,p.id asc ")
    ...
    List<String> findAllDistinctLanguageQuantity() ;

    @Query("select new com.manuel.curso.springboot.jpa.springbootjpa.dto.PersonDto(p.name, p.lastname) from Person p")
